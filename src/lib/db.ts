...
export async function connectToDB() {
  ...
  await db.connect();
  console.log(`Connecté à la base de données en mode ${isProduction ? 'production' : 'développement'}`);
...
export async function executeQuery(query: string, params?: (string | number | boolean | Date | null | number[])[]) {
  if (!db) {
    await connectToDB();
  }
